(function () {
  ...
  if (tbody) {
    ...
    if (!weeks.length) {
      tbody.innerHTML = '<tr><td colspan="5">Schedule data failed to load.</td></tr>';
    } else {
      var html = [];
      var holidayInserted = { holiday: false, march: false };
      weeks.forEach(function (w) {
        if (!holidayInserted.holiday && w.week === 14) {
          html.push(
            '<tr class="break-row"><td colspan="5">Holiday break — no games Dec 23 &amp; Dec 30</td></tr>'
          );
        ...
        html.push(
          "<tr>" +
            ...
            "<td>" +
            esc(w.date) +
            (w.label ? "<div class=\"muted-week\">" + esc(w.label) + "</div>" : "") +
            "</td>" +
            "<td>" +
  ...
  if (pbody && data.playoffs && data.playoffs.length) {
    ...
    data.playoffs.forEach(function (round) {
      round.games.forEach(function (g, i) {
        ...
        if (slotKey.indexOf("early") !== -1 || slotKey.indexOf("final") !== -1 || slotKey.indexOf("game 1") !== -1)
          slotKey = "early";
        else if (slotKey.indexOf("middle") !== -1 || slotKey.indexOf("game 2") !== -1)
          slotKey = "middle";
        else if (slotKey.indexOf("late") !== -1) slotKey = "late";
        else slotKey = "";
        var result = slotKey
          ? findResult(lookup, null, round.date, slotKey, g.home, g.away)
          : null;
